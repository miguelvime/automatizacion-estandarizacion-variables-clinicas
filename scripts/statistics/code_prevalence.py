...
# Lista de códigos a excluir del análisis por ser outliers/ubicuos
EXCLUDED_CODES = {'b280'} 

print(f"Iniciando procesamiento. Códigos excluidos del análisis: {EXCLUDED_CODES}")
...
try:
    ...
    print("Error crítico: El archivo JSON está corrupto.")
    exit(1)

# =====================================================================
# 2. EXTRACCIÓN FILTRADA Y MÉTRICAS
# =====================================================================
code_counter = Counter()
codes_per_record = []

for entry in data:
    raw_codes = entry.get("icf_codes", [])
    if isinstance(raw_codes, list):
        # Filtramos los códigos ubicuos antes de contarlos
        filtered_codes = [code for code in raw_codes if code not in EXCLUDED_CODES]
        
        if filtered_codes:
            code_counter.update(filtered_codes)
            codes_per_record.append(len(filtered_codes))
        else:
...
if total_extracted_codes == 0:
    print("Error: No se encontraron códigos válidos tras aplicar los filtros.")
    exit(1)
...
print(f"Códigos ICF únicos (sin b280): {total_unique_codes}")
print(f"Total de códigos extraídos válidos (denominador): {total_extracted_codes}")

# =====================================================================
# 3. CÁLCULO DE FRECUENCIAS RELATIVAS (Total = 100%)
# =====================================================================
all_codes = []
...
for code, count in code_counter.most_common():
    ...
    # Proporción exacta con respecto al volumen real de códigos filtrados
    rel_freq = (count / total_extracted_codes) * 100
    relative_frequencies.append(rel_freq)

# Verificación de integridad matemática
sum_freq = sum(relative_frequencies)
print(f"Suma de todas las proporciones calculadas: {sum_freq:.2f}%")
...
plt.title(f'Frecuencia Relativa Top {top_n} Códigos ICF (Excluyendo outliers)\n(Total de Códigos = {total_extracted_codes}, Códigos Únicos = {total_unique_codes})')
...
plt.tight_layout()
output_png = './data/results/frec_relativa_filtrada.png'
plt.savefig(output_png, dpi=300, bbox_inches='tight')
print(f"Gráfico exportado con éxito a: {output_png}")

# =====================================================================
# 5. GENERACIÓN DE TABLAS (EXCEL)
# =====================================================================
summary_data = {
    "Variable": [
        "Registros clínicos evaluados (N)",
        "Códigos excluidos del análisis",
        "Total de códigos ICF extraídos válidos (n)",
        "Códigos ICF únicos identificados (n)",
        "Códigos válidos por historia (Media ± DE)",
        "Códigos válidos por historia (Mediana)"
    ],
    "Valor": [
        total_records,
        ", ".join(EXCLUDED_CODES),
...
excel_output_path = './data/results/tablas_publicacion_filtradas.xlsx'
try:
    ...
    csv_output_path = './data/results/frec_relativa_filtrada.csv'
